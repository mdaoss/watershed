...
type EgressPolicyVal4 struct {
	...
	GatewayIP [MaxGatewayCount]types.IPv4 `align:"gateway_ip"`
}
...
type PolicyMap interface {
	...
	Update(sourceIP netip.Addr, destCIDR netip.Prefix, egressIP netip.Addr, gatewayIP [MaxGatewayCount]netip.Addr) error
...
// WS - Required to extend to support  3 gwIP
// NewEgressPolicyVal4 returns a new EgressPolicyVal4 object representing for
// the given egress IP and gateway IPs
func NewEgressPolicyVal4(egressIP netip.Addr, gatewayIPList [MaxGatewayCount]netip.Addr) EgressPolicyVal4 {
	val := EgressPolicyVal4{}

	val.EgressIP.FromAddr(egressIP)
	// WS - Optimize it via custom type ?
	val.GatewayIP[0].FromAddr(gatewayIPList[0])
	val.GatewayIP[1].FromAddr(gatewayIPList[1])
...
// policy value.
func (v *EgressPolicyVal4) Match(egressIP netip.Addr, gatewayIPList [MaxGatewayCount]netip.Addr) bool {
	a := v.GetEgressAddr() == egressIP
	c := v.GetGatewayAddrList()
	d := gatewayIPList
	b := (c[0].Compare(d[0]) == 0 &&
		c[1].Compare(d[1]) == 0)
...
// WS -- required to extend to work with multiple gwIP
// GetGatewayIP returns the egress policy value's gateway IP.
func (v *EgressPolicyVal4) GetGatewayAddrList() [MaxGatewayCount]netip.Addr {
	return [MaxGatewayCount]netip.Addr{
		v.GatewayIP[0].Addr(),
		v.GatewayIP[1].Addr(),
	}
}
...
func (v *EgressPolicyVal4) String() string {
	return fmt.Sprintf("%s %s", v.GetGatewayAddrList(), v.GetEgressAddr())
}
...
func (m *policyMap) Update(sourceIP netip.Addr, destCIDR netip.Prefix, egressIP netip.Addr, gatewayIPList [MaxGatewayCount]netip.Addr) error {
	key := NewEgressPolicyKey4(sourceIP, destCIDR)
	val := NewEgressPolicyVal4(egressIP, gatewayIPList)
