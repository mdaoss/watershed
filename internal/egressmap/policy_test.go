...
func TestPolicyMap(t *testing.T) {
	...
	err := egressPolicyMap.Update(sourceIP1, destCIDR1, egressIP1, [MaxGatewayCount]netip.Addr{egressIP1})
	assert.Nil(t, err)

	err = egressPolicyMap.Update(sourceIP2, destCIDR2, egressIP2, [MaxGatewayCount]netip.Addr{egressIP2})
	assert.Nil(t, err)
